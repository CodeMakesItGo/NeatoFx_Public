...
> run `esphome config Displays/NeatoDisplay7/main.yaml` and flash-test once to confirm
> the LVGL `shot_page` widget ids line up with `scripts/game_scripts.yaml`.

A modular ESPHome project for the 800×480 touch-screen game display station.
...
All images should be 800×480 PNG.
...
```bash
# Station 1 — Yellow
esphome -s id 1 -s color YLW -s font_color 0xFFE000 run Displays/NeatoDisplay7/main.yaml

# Station 4 — Green
esphome -s id 4 -s color GRN -s font_color 0x00E000 run Displays/NeatoDisplay7/main.yaml

# Station 2 — Blue
esphome -s id 2 -s color BLU -s font_color 0x00BFFF run Displays/NeatoDisplay7/main.yaml

# Station 3 — Orange
esphome -s id 3 -s color ORG -s font_color 0xFF6000 run Displays/NeatoDisplay7/main.yaml
```
